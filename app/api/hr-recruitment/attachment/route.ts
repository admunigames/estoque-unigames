...
// Termo de integração assinado + print da reunião de integração (Etapa 4) —
// mesmo fluxo em partes (create/chunk/complete/cancel) de
// app/api/hr-store-tracking/attachment/route.ts, com "kind" apontando qual
// dos dois pares de coluna recebe o anexo.
...
function candidateIdIsValid(value: string) {
  return /^[0-9a-f-]{36}$/i.test(value);
}

function stagingPrefix(sessionId: string) {
  return `hr-recruitment/_pending/${sessionId}`;
}
...
function attachmentMetadataError(metadata: StagedAttachment) {
  if (!COLUMN_PREFIX[metadata.kind]) return "TIPO DE ANEXO INVÁLIDO.";
  if (!candidateIdIsValid(metadata.candidateId)) return "CANDIDATO INVÁLIDO.";
...
async function createSession(payload: JsonMap) {
  const metadata: StagedAttachment = {
    ...
    candidateId: safeText(payload.id, 80),
  ...
  const existing = await database
    .prepare("SELECT id FROM hr_recruitment_candidates WHERE id=?1 LIMIT 1")
    .bind(metadata.candidateId)
    .first<{ id: string }>();
...
async function storeChunk(request: Request) {
  const sessionId = safeText(request.headers.get("x-hr-recruitment-upload-id"), 80);
  const partNumber = numberValue(request.headers.get("x-hr-recruitment-part-number"));
...
async function completeSession(payload: JsonMap, actor: Identity) {
  ...
  try {
    ...
    const prefix = COLUMN_PREFIX[metadata.kind];
    const database = await getD1();
    const existing = await database
      .prepare(
        `SELECT id, ${prefix}_r2_key AS r2Key FROM hr_recruitment_candidates WHERE id=?1 LIMIT 1`,
      )
      .bind(metadata.candidateId)
    ...
    const r2Key = `hr-recruitment/${metadata.kind}/${metadata.candidateId}/${safeR2FileName(metadata.fileName)}`;
      ...
      await database
        .prepare(
          `UPDATE hr_recruitment_candidates
           SET ${prefix}_file_name=?1, ${prefix}_r2_key=?2, ${prefix}_size_bytes=?3,
               updated_by=?4, updated_by_name=?5, updated_at=CURRENT_TIMESTAMP
           WHERE id=?6`,
        )
        .bind(metadata.fileName, r2Key, metadata.fileSize, actor.id, actorName(actor), metadata.candidateId)
        .run();
    } catch (error) {
    ...
    return jsonResponse({ attached: true, id: metadata.candidateId, fileName: metadata.fileName }, 201);
  } finally {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("Não foi possível processar o anexo do candidato.", error);
...
export async function DELETE(request: Request) {
  ...
  if (!prefix || !candidateIdIsValid(id)) return jsonResponse({ error: "REGISTRO INVÁLIDO." }, 400);

  try {
    const database = await getD1();
    const existing = await database
      .prepare(`SELECT ${prefix}_r2_key AS r2Key FROM hr_recruitment_candidates WHERE id=?1 LIMIT 1`)
      .bind(id)
      .first<{ r2Key: string }>();
    if (!existing) return jsonResponse({ error: "CANDIDATO NÃO ENCONTRADO." }, 404);
    await database
      .prepare(
        `UPDATE hr_recruitment_candidates
         SET ${prefix}_file_name='', ${prefix}_r2_key='', ${prefix}_size_bytes=0,
    ...
    console.error("Não foi possível remover o anexo do candidato.", error);
