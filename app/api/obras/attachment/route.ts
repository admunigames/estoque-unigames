...
// Anexo de nota/orçamento (PDF) de um lançamento de obra. Mesmo fluxo em
// partes (create/chunk/complete/cancel) das Notas de O.S. e do comprovante
// da Folha — o Worker tem limite de corpo por requisição, então o PDF sobe
// em pedaços de 512 KB para uma área de staging no R2 e só é montado/
// validado no "complete".
...
function stagingPrefix(sessionId: string) {
  return `obras/_pending/${sessionId}`;
}
...
function metadataError(metadata: Staged) {
  if (!UUID_PATTERN.test(metadata.entryId)) return "LANÇAMENTO DE OBRA INVÁLIDO.";
  ...
  if (metadata.fileSize > MAX_PDF_SIZE) return "O ANEXO DEVE TER NO MÁXIMO 25 MB.";
  return "";
}
...
async function readMetadata(bucket: R2Bucket, sessionId: string) {
  ...
  return JSON.parse(await object.text()) as Staged;
}
...
async function createSession(payload: JsonMap) {
  ...
  const existing = await database
    .prepare("SELECT id FROM obra_entries WHERE id=?1 LIMIT 1")
    .bind(metadata.entryId)
    .first<{ id: string }>();
  if (!existing) return jsonResponse({ error: "LANÇAMENTO DE OBRA NÃO ENCONTRADO." }, 404);
...
async function storeChunk(request: Request) {
  const sessionId = safeText(request.headers.get("x-obra-upload-id"), 80);
  const partNumber = numberValue(request.headers.get("x-obra-part-number"));
...
async function completeSession(payload: JsonMap, actor: Identity) {
  ...
  try {
    const metadata = await readMetadata(bucket, sessionId);
    if (!metadata) return jsonResponse({ error: "O ENVIO DO PDF EXPIROU. SELECIONE-O NOVAMENTE." }, 410);
    const err = metadataError(metadata);
    if (err) return jsonResponse({ error: err }, 400);

    const database = await getD1();
    const existing = await database
      .prepare("SELECT id, attachment_r2_key AS attachmentR2Key FROM obra_entries WHERE id=?1 LIMIT 1")
      .bind(metadata.entryId)
    ...
    const r2Key = `obras/${metadata.entryId}/${safeR2FileName(metadata.fileName)}`;
    await bucket.put(r2Key, bytes, {
      httpMetadata: { contentType: PDF_CONTENT_TYPE },
      customMetadata: { obraEntryId: metadata.entryId, uploadedBy: actor.id, uploadedAt: new Date().toISOString() },
    });
    ...
    try {
      await database
        ...
        .bind(metadata.fileName, r2Key, metadata.fileSize, metadata.entryId)
        .run();
    } catch (error) {
...
export async function POST(request: Request) {
  ...
  if (!canManageWorks(actor)) {
    return jsonResponse({ error: "VOCÊ NÃO TEM PERMISSÃO PARA ANEXAR NOTAS DE OBRA." }, 403);
  }
  ...
  try {
    ...
    if (contentType.includes("application/octet-stream")) return await storeChunk(request);
    return jsonResponse({ error: "TIPO DE REQUISIÇÃO INVÁLIDO." }, 400);
  } catch (error) {
    console.error("Não foi possível processar o anexo da obra.", error);
...
export async function DELETE(request: Request) {
  ...
  const actor = identity(request);
  if (!canManageWorks(actor)) {
    return jsonResponse({ error: "VOCÊ NÃO TEM PERMISSÃO PARA REMOVER ANEXOS DE OBRA." }, 403);
  }
  if (!sameOrigin(request)) {
    return jsonResponse({ error: "ORIGEM NÃO PERMITIDA." }, 403);
  }
  const id = safeText(new URL(request.url).searchParams.get("id"), 80);
  if (!UUID_PATTERN.test(id)) return jsonResponse({ error: "LANÇAMENTO DE OBRA INVÁLIDO." }, 400);
  ...
  try {
    ...
    console.error("Não foi possível remover o anexo da obra.", error);
