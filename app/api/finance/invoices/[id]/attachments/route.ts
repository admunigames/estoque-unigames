...
import { assertInvoiceAccess, canReconcileInvoices, loadInvoice } from "../../shared";

type StagedAttachment = {
  invoiceId: string;
  installmentId: string;
  paymentId: string;
  attachmentType: "nf" | "boleto" | "comprovante";
  fileName: string;
  contentType: string;
  fileSize: number;
...
const ATTACHMENT_TYPES = new Set(["nf", "boleto", "comprovante"]);
...
function stagingPrefix(sessionId: string) {
  return `supplier-invoices/_pending/${sessionId}`;
}
...
function metadataError(metadata: StagedAttachment) {
  ...
  const refCount = [metadata.installmentId, metadata.paymentId].filter(Boolean).length;
  if (refCount > 1) return "ANEXO INVÁLIDO (MAIS DE UMA REFERÊNCIA)."
...
async function createSession(payload: JsonMap) {
  ...
  const metadata: StagedAttachment = {
    invoiceId: safeText(payload.invoiceId, 80),
    installmentId: safeText(payload.installmentId, 80),
  ...
  const invoice = await loadInvoice(database, metadata.invoiceId);
...
async function completeSession(payload: JsonMap, actor: { id: string; displayName: string }) {
  ...
  try {
    ...
    const targetSegment = metadata.installmentId || metadata.paymentId || "invoice";
    const r2Key = `supplier-invoices/${metadata.invoiceId}/${targetSegment}/${attachmentId}-${safeR2FileName(metadata.fileName)}`;

    await bucket.put(r2Key, bytes, {
      httpMetadata: { contentType: PDF_CONTENT_TYPE },
      customMetadata: { invoiceId: metadata.invoiceId, uploadedBy: actor.id, uploadedAt: new Date().toISOString() },
    });
    ...
    try {
      await database
        ...
        .bind(
          attachmentId,
          metadata.installmentId || metadata.paymentId ? "" : metadata.invoiceId,
    ...
    await database
      .prepare(
        `INSERT INTO supplier_invoice_events (id, invoice_id, event_type, description, metadata_json, actor_id, actor_name, created_at)
         VALUES (?1,?2,'attachment_added',?3,?4,?5,?6,CURRENT_TIMESTAMP)`,
      )
      .bind(
        crypto.randomUUID(),
        metadata.invoiceId,
        `ARQUIVO ANEXADO (${metadata.attachmentType.toUpperCase()}): ${metadata.fileName}`,
        JSON.stringify({ attachmentId, attachmentType: metadata.attachmentType }),
        actor.id,
        actor.displayName || "Usuário",
      )
      .run();
...
/**
 * Núcleo do upload de anexo de NF/duplicata/pagamento — reaproveitado tanto
 * pela rota do Financeiro (permissão payables:invoices_reconcile) quanto
 * pela rota de Compras (permissão purchases:edit, só para o arquivo da
 * própria NF antes do envio), que fazem sua própria checagem de permissão
 * e autorização antes de chamar isto.
 */
export async function handleInvoiceAttachmentUpload(
  request: Request,
  invoiceId: string,
  ...
  try {
    ...
    console.error("Não foi possível processar o anexo da nota fiscal.", error);
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return handleInvoiceAttachmentUpload(request, id, actor);
}
