...
const SUNDAY_WORK_COLUMNS = `
  ...
  reference_month AS referenceMonth, work_date AS workDate, work_time AS workTime,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const workTime = safeText(body.workTime, 10);
    ...
    await database
      .prepare(
        `INSERT INTO hr_schedule_sunday_work
          (id, employee_id, employee_name, reference_month, work_date, work_time,
           created_by, created_by_name, created_at, updated_by, updated_by_name, updated_at)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, CURRENT_TIMESTAMP, ?7, ?8, CURRENT_TIMESTAMP)`,
      )
      .bind(recordId, employeeId, employeeName, referenceMonth, workDate, workTime, actor.id, actorName(actor))
      .run();
